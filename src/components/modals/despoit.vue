  ...
  <transition name="modal" :class="chainName">
    <div class="model" v-if="modelShow">
      <div class="model-wrap text">
        <img class="close-icon" src="../../assets/close-icon.png" @click="hideModel" />
        <h3>{{ $t("Deposit") }} {{ data.name }}</h3>
        <div class="input-wrap numbers">
          <input v-model="value" type="text" />
          <div class="button-wrap">
            <button :disabled="!max" @click="() => this.setMax()">MAX</button>
          </div>
        </div>
        <p class="numbers">{{ max }} {{ data.name }} {{ $t("Available") }}</p>
        <div class="btn">
          <div class="cancel" @click="this.hideModel">{{ $t("Cancel") }}</div>
          <button class="confirm" @click="despoit" :disabled="!Number(value) || Number(value) > Number(max)">
            {{ $t("Confirm") }}
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async despoit() {
      ...
      this.hideModel();
      const despoitmsg = `Deposit ${this.value} ${this.data.name}`;
      const overalltip = this.$overalltip({
        type: "loading",
        msg: despoitmsg,
      });
      despoitValue(this.pid, this.value, this.account)
        .then((res) => {
          this.$emit("updateData");
          overalltip.close();
          const msg = `${despoitmsg} Success`;
